...
# Статус берём с доски (Projects), а не из меток — единый источник правды
declare -A BST
...
st_name(){ # $1=num $2=labels
  local b="${BST[$1]:-}"
  case "$b" in
    "In Progress") echo "🔄 В работе"; return;;
    "Done") echo "✅ Готово"; return;;
  esac
...
while IFS=$'\t' read -r num title labels ms b64; do
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$(pr_rank "$labels")" "$num" "${title}${mark}" "$(pr_name "$labels")" "$(st_name "$num" "$labels")" "$when" "${proj:-—}" "$(cat_name "$labels")" >> "$ROWS"
done < "$TMP"
